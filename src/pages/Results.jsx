    ...
    const categories = [
      {
        ...
        label: "Activity",
        cssColor: "#3b82f6",
        textColor: "text-cat-activity",
      },
      {
        key: "skillSignals",
        label: "Skill Signals",
        cssColor: "#a855f7",
        textColor: "text-cat-skills",
      },
      {
        key: "growth",
        label: "Growth",
        cssColor: "#22c55e",
        textColor: "text-cat-growth",
      },
      {
        key: "collaboration",
        label: "Collaboration",
        cssColor: "#eab308",
        textColor: "text-cat-collab",
      },
    ];
    const ringColor =
      analysis.cookedLevel >= 9
        ? "#22c55e"
        : analysis.cookedLevel >= 7
          ? "#eab308"
          : analysis.cookedLevel >= 5
            ? "#f97316"
            : analysis.cookedLevel >= 3
              ? "#ef4444"
              : "#dc2626";
    return (
      <>
      ...
      <header className="border-b border-border bg-background-dark">
        <div className="max-w-full mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-2 sm:gap-4">
          ...
          >
            ...
            />
            <h1 className="text-lg sm:text-2xl font-bold text-foreground hidden sm:block">
              AmICooked?
            </h1>
          </button>
          ...
          <div className="relative shrink-0" ref={profileMenuRef}>
            ...
            {profileMenuOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-card border border-border rounded-lg shadow-xl py-1.5 z-50">
                {/* Profile */}
                <p className="text-[10px] uppercase tracking-widest text-muted-foreground/50 px-4 pt-2 pb-1">
                  Profile
                </p>
                <button
                  onClick={() => {
                ...
                <p className="text-[10px] uppercase tracking-widest text-muted-foreground/50 px-4 pb-1">
                  Analysis
                </p>
                <button
                  onClick={() => {
            ...
            <Card className="bg-background border-none lg:sticky lg:top-8">
              <CardContent className="pt-6">
                <div className="flex flex-col items-center lg:items-start text-center lg:text-left mb-6">
                  ...
                  />
                  <div className="flex flex-col lg:items-start xl:flex-row xl:items-center gap-1 xl:gap-3 items-center">
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="flex flex-col items-center gap-1 shrink-0">
                            ...
                            <span
                              className={`text-xs font-semibold ${getCookedColor(analysis.cookedLevel)}`}
                            >
                              {analysis.levelName}
                            </span>
                    ...
                    {(recommendedProjects || []).slice(0, 4).map((rec, idx) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          ...
                          >
                            <Bookmark
                              className={`w-3.5 h-3.5 ${savedProjectNames.has(rec.name) ? "fill-current" : ""}`}
                            />
                          </button>
                        </div>
                        ...
                        <>
                          {(() => {
                            ...
                            heatmap.weeks.forEach((week, col) => {
                              ...
                              if (
                                !monthMarkers.length ||
                                monthMarkers[monthMarkers.length - 1].name !==
                                  month
                              ) {
                                const prev =
                                  monthMarkers[monthMarkers.length - 1];
                  ...
                  <div className="w-full md:w-[190px] shrink-0 border-t border-border md:border-t-0 md:border-l md:pl-6 pt-4 md:pt-0 mt-4 md:mt-0 flex flex-col justify-center">
                    <div className="space-y-[10px] text-xs text-muted-foreground">
                      {[
                        {
                          label: "Commits (365d)",
                          value: githubData.commitsLast365,
                          tip: "Total commit contributions in the last 365 days",
                        },
                        {
                          label: "Active Weeks",
                          value: `${githubData.activeWeeksPct}%`,
                          tip: "% of the past 52 weeks with at least one contribution",
                        },
                        {
                          label: "Streak",
                          value: `${githubData.streak}d`,
                          tip: "Current consecutive days with at least one contribution",
                        },
                        {
                          label: "Velocity Trend",
                          value:
                            githubData.commitVelocityTrend >= 1
                              ? `↑ ${githubData.commitVelocityTrend}×`
                              : `↓ ${githubData.commitVelocityTrend}×`,
                          tip: "Commits this year ÷ last year — >1 means accelerating",
                        },
                        {
                          label: "Merged PRs",
                          value: `${githubData.mergedPRs} / ${githubData.totalPRs}`,
                          tip: "Merged pull requests out of total PRs opened",
                        },
                      ].map((item, i) => (
                        <div
                          key={i}
          ...
          <div className="flex items-center justify-between px-8 py-5 border-b border-border shrink-0">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                Every metric fed to the AI to generate your analysis
              </p>
            </div>
            <button
                ...
                {[
                  {
                    label: "Commits (365d)",
                    value: githubData.commitsLast365,
                    desc: "Commit contributions this year",
                  },
                  {
                    label: "Commits (90d)",
                    value: githubData.commitsLast90,
                    desc: "Contributions in the last 90 days",
                  },
                  {
                    label: "Prev Year Commits",
                    value: githubData.prevYearCommits,
                    desc: "Commits in the year prior — baseline for velocity",
                  },
                  {
                    label: "Active Weeks",
                    value: `${githubData.activeWeeksPct}%`,
                    desc: "% of 52 weeks with ≥1 contribution",
                  },
                  {
                    label: "Avg / Active Week",
                    value: githubData.avgCommitsPerActiveWeek,
                    desc: "Commits per week when active",
                  },
                  {
                    label: "Std Dev / Week",
                    value: githubData.stdDevPerWeek,
                    desc: "Lower = more consistent cadence",
                  },
                  {
                    label: "Longest Inactive Gap",
                    value: `${githubData.longestInactiveGap}d`,
                    desc: "Longest stretch without a contribution",
                  },
                  {
                    label: "Current Streak",
                    value: `${githubData.streak}d`,
                    desc: "Consecutive days with contributions",
                  },
                  {
                    label: "Total Contributions",
                    value: githubData.totalContributions,
                    desc: "All GitHub contribution events this year",
                  },
                ].map((s, i) => (
                  <div
                    key={i}
                ...
                {[
                  {
                    ...
                    value: githubData.languageCount,
                    desc: "Unique languages across all repos",
                  },
                  {
                    label: "Top Lang Dominance",
                    value: `${githubData.topLanguageDominancePct}%`,
                    desc: "% of codebase bytes in your #1 language",
                  },
                  {
                    label: "Total Language Bytes",
                    value: githubData.totalLanguageBytes?.toLocaleString(),
                    desc: "Total bytes of code across all repos",
                  },
                ].map((s, i) => (
                  <div
                    key={i}
                        ...
                        .map(([cat, pct], i) => (
                          <div key={i} className="flex items-center gap-3">
                            ...
                            <div className="flex-1 h-1.5 bg-track rounded-full overflow-hidden">
                              <div
                                className="h-full bg-cat-skills/60 rounded-full"
                                style={{ width: `${pct}%` }}
                              />
                            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {[
                  {
                    label: "Velocity Trend",
                    value: `${githubData.commitVelocityTrend}×`,
                    desc: "Commits this year ÷ last year — >1 accelerating",
                  },
                  {
                    label: "Momentum Ratio",
                    value: githubData.activityMomentumRatio,
                    desc: "(Last 90d × 4) ÷ last 365d — >1 ramping up",
                  },
                  {
                    label: "Domain Diversity Δ",
                    value:
                      githubData.domainDiversityChange > 0
                        ? `+${githubData.domainDiversityChange}`
                        : githubData.domainDiversityChange,
                    desc: "New tech domains explored vs prior year",
                  },
                ].map((s, i) => (
                  <div
                    key={i}
                      ...
                      className={`text-base font-bold ${
                        s.label === "Velocity Trend"
                          ? githubData.commitVelocityTrend >= 1
                            ? "text-green-400"
                            : "text-red-400"
                          : s.label === "Momentum Ratio"
                            ? githubData.activityMomentumRatio >= 1
                              ? "text-green-400"
                              : "text-orange-400"
                            : s.label === "Domain Diversity Δ"
                              ? githubData.domainDiversityChange > 0
                                ? "text-green-400"
                                : githubData.domainDiversityChange < 0
                                  ? "text-red-400"
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {[
                  {
                    label: "Total PRs",
                    value: githubData.totalPRs,
                    desc: "All pull requests ever opened",
                  },
                  {
                    label: "Merged PRs",
                    value: githubData.mergedPRs,
                    desc: "Pull requests that were merged",
                  },
                  {
                    label: "Open Issues",
                    value: githubData.openIssues,
                    desc: "Currently open issues",
                  },
                  {
                    label: "Closed Issues",
                    value: githubData.closedIssues,
                    desc: "Issues resolved",
                  },
                  {
                    label: "Issues Closed Ratio",
                    value: githubData.issuesClosedRatio,
                    desc: "Closed ÷ (open + 1) — higher is better",
                  },
                ].map((s, i) => (
                  <div
                    key={i}
                ...
                {[
                  {
                    ...
                    desc: "Public repos owned",
                  },
                  {
                    label: "Total Stars",
                    value: githubData.totalStars,
                    desc: "Stars received across all repos",
                  },
                  {
                    label: "Total Forks",
                    value: githubData.totalForks,
                    desc: "Times your repos were forked",
                  },
                  {
                    label: "Commits in Repos",
                    value: githubData.totalCommitsInRepos?.toLocaleString(),
                    desc: "Total commits across all repo histories",
                  },
                ].map((s, i) => (
                  <div
                    key={i}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-xs leading-relaxed">
                AI-powered GitHub profile analysis to help you level up your
                developer career.
              </p>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2 text-xs">
                ...
                <li>
                  <a
                    href="https://github.com/Champion2005/amicooked/issues"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-muted-foreground hover:text-accent transition-colors"
                  >
